export class Leaderboard extends Phaser.GameObjects.Group {
    constructor(scene, x, y, pageNum) {
        ...
        let bg = scene.add.image(x, y + 20, 'board').setVisible(false).setScale(3);
        this.add(bg);

        for(let i = 0; i < pageNum; i++) {
            this.titles[i] = scene.add.text(this.startX, y, "test" + i, {
                fontFamily: 'Arial',
                fontStyle: 'bold',
                fontSize: '32px',
                color:  '#0000EE',
                ...
                align: 'center'
            }).setInteractive({ useHandCursor: true }).setVisible(false).setOrigin(0.5, 0.5);
            ...
            for (let j = 0; j < this.listItems; j++) {
                let listLine = scene.add.text(x, this.startY, "test body " + i + " " + j, {
                    fontFamily: 'Arial',
                    fontStyle: 'bold',
                    fontSize: '24px',
                    color:  '#ffffff',
                    ...
                    align: 'center'
                }).setOrigin(0.5, 0).setVisible(false).setStroke('#000000', 3);
