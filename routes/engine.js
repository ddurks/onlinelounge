...
class Item {
  constructor(world, x, y, itemType) {
    let itemBody = Matter.Bodies.rectangle(x, y, 32, 32, {
      itemType: itemType,
    });
...
class GameEngine {
  constructor(io) {
    ...
    tmx.parseFile(
      "./public/assets/tiles/onlinepluto-tilemap-new.tmx",
      (err, map) => {
        if (err) throw err;
        var tileMap = map;
        var currX = 0,
    ...
    tmx.parseFile(
      "./public/assets/tiles/onlinelounge-tilemap.tmx",
      (err, map) => {
        if (err) throw err;
        var tileMap = map;
        var currX = 0,
    ...
    Matter.Events.on(this.engine, "collisionStart", (event) => {
      ...
      if (bodyA.isCoin || bodyB.isCoin) {
        if (
          bodyA.isCoin &&
          bodyB.socketId &&
          bodyA.droppedBy !== bodyB.socketId
        ) {
        ...
        if (
          bodyB.isCoin &&
          bodyA.socketId &&
          bodyB.droppedBy !== bodyA.socketId
        ) {
  ...
  hitByBullet(player, firedBy) {
    if (player.health > 0) {
      ...
      if (player.health === 0) {
        ...
        let slayer,
          firedByPlayer = this.players.get(firedBy);
        if (firedByPlayer) {
          slayer = firedByPlayer.username
            ? firedByPlayer.username
            : "[anonymous]";
          firedByPlayer.kills++;
          this.leaderboard.addStats(
            {
        ...
        this.io.sockets.emit("player action", {
  ...
  update() {
    if (this.players.size > 0) {
      ...
      this.engineRateAvg = this.approxRollingAverage(
        this.engineRateAvg,
        this.deltat
      );
  ...
  spawnRandomItem() {
    if (this.items.size < MAX_ITEMS) {
      this.spawnItem(
        this.engine.world,
        this.getRandomNum(50, this.planetWidth - 50),
        this.getRandomNum(50, this.planetHeight - 50),
  ...
  addPlayer(player) {
    ...
    if (!this.players.has(player.id)) {
      this.players.set(
        player.id,
        this.Bodies.rectangle(player.x, player.y, player.width, player.height, {
          width: player.width,
      ...
      this.Composite.add(
        player.currentArea === AREAS.digitalplanet
          ? this.engine.world
          : this.loungeEngine.world,
        newPlayer
      );
  ...
  digForTreasure(socketId) {
    ...
    if (player && player.currentArea === AREAS.digitalplanet) {
      ...
      this.treasures.forEach((treasure, index) => {
        if (
          player.position.x >= treasure.x - radius &&
          player.position.x <= treasure.x + radius &&
          player.position.y >= treasure.y - radius &&
          player.position.y <= treasure.y + radius
        ) {
  ...
  spawnItem(world, x, y, type) {
    ...
    this.io.sockets.emit("item", {
      spawn: { itemId: newItem.itemId, x: x, y: y, itemType: type },
    });
  ...
  addBlock(world, x, y, width, height) {
    let newBlock = this.Bodies.rectangle(
      x + width / 2,
      y + height / 2,
      width,
      height,
  ...
  shootBullet(socketId, direction) {
    ...
    if (
      ...
      let newBullet = this.Bodies.rectangle(pos.x, pos.y, 16, 16, {
        isBullet: true,
        direction: direction,
      });
  ...
  dropCoins(socketId, amount) {
    ...
    if (
      ...
      for (let i = 0; i < amount; i++) {
        let newCoin = this.Bodies.rectangle(
          player.position.x,
          player.position.y,
          16,
          16,
          { isCoin: true, droppedBy: player.socketId }
        );
        newCoin.itemId = uuidv4();
        newCoin.frictionAir = 0.4;
        this.Composite.add(this.engine.world, newCoin);
        this.Body.setVelocity(newCoin, {
          x: this.getRandomNum(-COIN_SPEED, COIN_SPEED),
          y: this.getRandomNum(-COIN_SPEED, COIN_SPEED),
        });
  ...
  executeCommand(socketId, command) {
    ...
    if (commandPlayer) {
      ...
      if (command === "/bullets$") {
        ...
        this.io
          .to(commandPlayer.socketId)
          .emit("bullet update", commandPlayer.bullets);
      }
  ...
  handleInputState(player) {
    if (player.currentInputs) {
      ...
      if (
        player.currentInputs[Key.a] === 1 ||
        player.currentInputs[Key.d] === 1 ||
        player.currentInputs[Key.w] === 1 ||
        player.currentInputs[Key.s] === 1
      ) {
        if (player.health === 0) {
          player.health = 3;
          this.io.to(player.socketId).emit("health update", player.health);
  ...
  setPlayerLook(socketId, lookIndex) {
    ...
    if (playerToChange) {
      ...
      this.io.sockets.emit("player action", {
        socketId: socketId,
        actions: { lookIndex: playerToChange.lookIndex },
      });
    }
  }
  ...
  removePlayer(socketId) {
    if (this.players.has(socketId)) {
      ...
      console.log(
        "player left: " + socketId + " players: " + this.players.size
      );
      return username;
    } else {
      return false;
  ...
  getStats() {
    return {
      ...
      engineTick:
        Math.round((1000.0 / this.engineRateAvg + Number.EPSILON) * 100) / 100,
    };
  }

  getLeaderboard() {
    return Object.entries(this.leaderboard).reduce(
      (acc, [statName, statList]) => {
        if (statName !== "date") {
          acc[statName] = statList.map((stat) =>
            stat.number > 0 ? stat.username + " - " + stat.number : ""
          );
        }
        return acc;
      },
