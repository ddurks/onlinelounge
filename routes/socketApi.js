...
io.on("connection", (socket) => {
  ...
  socket.on("new player", (newPlayer) => {
    ...
    console.log(
      "adding " + socket.id + " players connected: " + engine.players.size,
      newPlayer.currentArea
    );
  });
  ...
  socket.on("player action", (actionArray) => {
    ...
    if (actionArray.typing) {
      io.sockets.emit("player action", {
        socketId: socket.id,
        actions: { typing: actionArray.typing },
      });
    }
    if (actionArray.message && actionArray.message.startsWith("/")) {
      ...
      io.sockets.emit("player action", {
        socketId: socket.id,
        actions: { message: "NULL", commandResult: result },
      });
  ...
  socket.on("server stats", () => {
    ...
    serverStats.serverTick =
      Math.round((1000.0 / tickRateAvg + Number.EPSILON) * 100) / 100;
...
setInterval(() => {
  if (engine.players.size > 0) {
    ...
    currUpdateTimestamp = Date.now();
    tickRateAvg = approxRollingAverage(
      tickRateAvg,
      currUpdateTimestamp - lastUpdateTimestamp
    );
